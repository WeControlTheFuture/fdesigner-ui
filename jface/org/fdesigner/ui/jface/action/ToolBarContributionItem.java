...
import org.fdesigner.ui.jface.util.Assert;
...
public class ToolBarContributionItem extends ContributionItem implements IToolBarContributionItem {
	 ...
	 * <code>getMinimumItemsToShow</code> to indicate that all tool items should be
	 * shown in the cool item.
	 */
	public static final int SHOW_ALL_ITEMS = -1;

	/**
	 * The pull down menu used to list all hidden tool items if the current size is
	 * less than the preffered size.
	 */
	...
	/**
	 * The tool bar manager used to manage the tool items contained in the cool item
	 * widget.
	 */
	...
	public void dispose() {
		...
		/*
		 * We need to dispose the cool item or we might be left holding a cool item with
		 * a disposed control.
		 */
	...
	private void handleChevron(SelectionEvent event) {
		...
		for (ToolItem toolItem : items) {
			...
			if (!((itemBounds.x + itemBounds.width <= toolBarBounds.width)
					&& (itemBounds.y + itemBounds.height <= toolBarBounds.height))) {
				hidden.add(toolItem);
			}
		}
	 ...
	 * separators.
	 *
	 * @return <code>true</code> if the tool bar manager contains something other
	 *         than group marks and separators, and the internal state is set to be
	 *         visible.
	 */
	@Override
	 ...
	 * hidden tool items appear in a drop down menu. By default, all the tool items
	 * are shown in the cool item.
	 *
	 ...
	 * support is enabled.
	 *
	 * @param value <code>true</code> to enable chevron support, <code>false</code>
	 *              otherwise.
	 */
	@Override
	 ...
	 *                          changed to the preferred size, <code>false</code> to
	 *                          not change the current size
	 */
